...
static void aim_app_activate (GApplication *app)
{
    ...
    g_signal_connect(GTK_STATUS_ICON (klass->systray), "button-press-event", G_CALLBACK (aim_app_on_systray_press), imwin);

    gdk_threads_add_timeout_seconds (3, aim_app_on_hide_imwin, imwin);
    gui_activate_callback(xid, dsy);
}
...
gboolean aim_app_on_switch_lan(gpointer user_data)
{
    ...
    aim_win_switch_lan(klass->imwin, *((gboolean *)user_data));
    return false;
}
