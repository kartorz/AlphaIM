...
#endif
{
    ...
    _XUnregisterInternalConnection(i18n_core->address.dpy, fd);
    TRANS(Disconnect)(spec->trans_conn);
    (void)TRANS(Close)(spec->trans_conn);
    return True;
}
...
#endif
{
    ...
    if ((len = TRANS(Read)(accept_conn, buf, buf_len)) <= 0)
      return False;
    *ret_len = len;
    return True;
}
...
#endif
{
    ...
    tr_client->accept_fd = TRANS(GetConnectionNumber)(accept_conn);
