...
char *argv[];
{
	...
	XNextEvent(dpy, &event);   // printf("XNextEvent %d\n", event.type);
	if (XFilterEvent(&event, None))
		...
		XwcDrawString(dpy, win, fontset, gc, 10, 50, string, str_len);
            break;
        case KeyPress:
		printf("i18n event keypress\n");
	    len = XwcLookupString(ic, &event, buffer, buf_len,
				  &keysym, &status);
            printf("keysym: %d, buf_len %d\n", keysym, len);
	    /*
	    ...
	    redraw = False;
	    //wprintf(L"XwcLooupString done %d -> %s\n", len, buffer);
            //printf("XwcLooupString   len:%d\n", len);
	    switch (status) {
	    case XLookupNone:
		break;
	    case XLookupKeySym:
		...
		XwcDrawString(dpy, win, fontset, gc, 10, 50, string, str_len);
        printf("redraw string: %d\n", str_len);
	    }
	    break;
	case ConfigureNotify:
		printf("i18n event configure notify\n");
	    /*
		...
		SetGeometry(ic, XNPreeditAttributes, &preedit_area);printf("Set preeditArea geometry\n");
	    }
		...
		SetGeometry(ic, XNStatusAttributes, &status_area);printf("Set statusArea geometry\n");
	    }
	    break;
        }
    }
