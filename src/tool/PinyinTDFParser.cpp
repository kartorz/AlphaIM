...
{
    m_writer.write(dbpath, m_header);
}

void PinyinTDFParser::parseInxtreeHeaderStart()
{
    memset(&m_header, 0, sizeof(struct inxtree_header));
    m_header.magic[0] = 0xB3;
    m_header.magic[1] = 0xB4;
    m_header.d_coding[0] = INXTREE_UTF8;
}
...
void PinyinTDFParser::parseInxtreeHeader(vector<string>& columns)
{
    if (columns.size() == 3) {
        if (columns[1] == "identify")
            strncpy((char *)(m_header.d_identi), columns[2].c_str(), 59);
        else if (columns[1] == "version") {
            inxtree_helper_w_version(m_header, columns[2]);
        }
    }
